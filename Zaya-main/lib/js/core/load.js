...
function updatePdfContext(pdfUrl, pdfId) {
    ...
    if (pdfUrl.startsWith('blob:')) {
        ...
        pdfType = 'url';
        pdfName = getPdfDisplayName(pdfUrl, pdfId);
        storageValue = pdfUrl;
    }
...
function updatePdfInfoDisplay(pdfName, pdfType) {
    ...
    if (pdfName) {
        document.title = `${pdfName} • The ID`;
    } else if (pdfType === 'local') {
        document.title = 'Local PDF • The ID';
    } else {
        document.title = 'The ID Viewer';
    }
}

function getPdfDisplayName(pdfUrl, pdfId) {
    if (pdfId && !pdfId.startsWith('http') && !pdfId.includes('/') && !pdfId.startsWith('blob:')) {
        return pdfId;
    }

    try {
        const resolvedUrl = new URL(pdfUrl, window.location.href);
        const fileName = resolvedUrl.pathname.split('/').pop() || resolvedUrl.hostname || pdfUrl;
        const decodedName = decodeURIComponent(fileName)
            .replace(/\.pdf$/i, '')
            .replace(/[-_]+/g, ' ')
            .trim();

        return decodedName || resolvedUrl.hostname || 'PDF';
    } catch (e) {
        return pdfUrl
            .replace(/\.pdf$/i, '')
            .split('/')
